...
def test_register_duplicate_email_returns_400(client):
    ...
    response = client.post(
        "/auth/register",
        json={"name": "Other", "email": "user@example.com", "password": "otherpassword"},
    )
...
# ---------------------------------------------------------------------------
# /cart/me — regression test for the route-ordering bug
#
# "/cart/me" only works correctly if it's registered BEFORE
# "/cart/{session_id}" in main.py. Otherwise, FastAPI treats "me"
# as a literal session_id and authentication is ignored entirely.
# ---------------------------------------------------------------------------
...
def test_cart_me_with_invalid_token_is_rejected_not_treated_as_guest_session(client):
    """Without this protection, '/cart/me' with an invalid token falls through to the
    '/cart/{session_id}' route (session_id='me') and returns 200 with an empty
    guest cart, instead of blocking access."""
    response = client.get("/cart/me", headers=auth_headers("invalid-token"))
    assert response.status_code == 401
...
# ---------------------------------------------------------------------------
# Merging the guest cart on login
# ---------------------------------------------------------------------------
...
def test_login_merges_guest_cart_into_user_cart(client, seeded_product):
    ...
    login_response = client.post(
        "/auth/login",
        json={"email": "merge@example.com", "password": "password123", "session_id": "guest-session"},
    )
    # user doesn't exist yet -> 401; register and try again
    assert login_response.status_code == 401
    register_user(client, email="merge@example.com", password="password123")
...
def test_login_merge_increments_quantity_for_overlapping_product(client, seeded_product):
    data = register_user(client, email="merge2@example.com", password="password123")
    client.post(
        "/cart/me/add",
        json={"product_id": "red-suit", "quantity": 1},
    ...
    login_response = client.post(
        "/auth/login",
        json={"email": "merge2@example.com", "password": "password123", "session_id": "guest-session-2"},
    )
    token = login_response.json()["access_token"]
...
def test_login_without_session_id_does_not_touch_other_guest_carts(client, seeded_product):
    register_user(client, email="nomerge@example.com", password="password123")
    client.post("/cart/untouched-session/add", json={"product_id": "red-suit", "quantity": 5})

    client.post("/auth/login", json={"email": "nomerge@example.com", "password": "password123"})
